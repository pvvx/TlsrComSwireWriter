...
def sws_rd_addr(addr):
	return sws_encode_blk(bytearray([0x5a, (addr>>8)&0xff, addr & 0xff, 0x80]))
...
def sws_wr_addr(addr, data):
	return sws_encode_blk(bytearray([0x5a, (addr>>8)&0xff, addr & 0xff, 0x00]) + bytearray(data)) + sws_encode_blk([0xff])
